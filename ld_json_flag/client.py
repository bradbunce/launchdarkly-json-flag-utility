...
class LaunchDarklyClient:
    ...
    def configure_environment_targeting(self, flag_key, environment_key, targeting_rules, project_key=None):
        """
        ...
        if response.status_code >= 400:
            print(f"❌ Error updating targeting rules for environment {environment_key}: {response.status_code}")
            print(f"Response: {response.text}")
            response.raise_for_status()

        print(f"✅ Targeting rules for '{flag_key}' in environment '{environment_key}' updated successfully")
        return response.json()
