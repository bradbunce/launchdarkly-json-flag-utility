...
def select_project(client):
    ...
    selected = select_from_list(
        projects, "\nAvailable projects:", lambda p: f"{p['name']} (key: {p['key']})"
    )
...
def edit_json_in_editor(json_data):
    ...
    print(
        "3. The value must contain a tcp_port property with a valid port number (0-65535)"
    )
    ...
    print(
        "6. When adding a new variation, you can omit the _id field - it will be generated automatically"
    )
...
def update_flag_variations_workflow(client, project_key=None):
    ...
    """
    # Use provided project_key, client's project_key, or prompt for selection
    project_key = project_key or client.project_key
    if not project_key:
    ...
    print(
        "You can edit, add, or remove variations. Please follow the instructions below."
    )
...
def create_flag_workflow(
    client, flag_key, flag_name, variations_file, env_rules=None, project_key=None
):
    """
    ...
    try:
        client.create_feature_flag(flag_key, flag_name, variations, project_key)
    except Exception as e:
        ...
        for env_rule in env_rules:
            try:
                ...
                client.configure_environment_targeting(
                    flag_key, env, targeting_rules, project_key
                )
            except ValueError:
...
def interactive_workflow(client):
    ...
    Returns:
        bool: True if successful, False otherwise
    """
    # Use client's project_key if set, otherwise prompt for selection
    ...
    # Ask if the user wants to create, update, or validate flags
    print("\nWhat would you like to do?")
    print("1. Create a new JSON feature flag")
    print("2. Update an existing JSON feature flag")
    print("3. Validate existing JSON feature flags")
    ...
    while True:
        ...
        if choice == "1":
            ...
            flag_key = (
                input(f"Enter feature flag key (suggested: {suggested_key}): ")
                or suggested_key
            )
            if not flag_key:
            ...
            try:
                ...
                if not environments:
                    ...
                    print(
                        f"Created template with {len(template_variations)} variations based on project environments."
                    )
            except Exception as e:
            ...
            try:
                ...
                return create_flag_workflow(
                    client, flag_key, flag_name, variations_file, None, project_key
                )
            finally:
            ...
            # Ask if the user wants to fix invalid flags
            fix_option = input("\nFix invalid flags if found? (y/n): ")
            fix_invalid = fix_option.lower() == "y"

            # Run the validate workflow with the selected option
            return validate_flags_workflow(client, fix_invalid, project_key)
        else:
            print("Please enter 1, 2, or 3")


def validate_flags_workflow(client, fix_invalid=False, project_key=None):
    """
    Validate all JSON feature flags in a project.

    Args:
        client (LaunchDarklyClient): LaunchDarkly client
        fix_invalid (bool): Whether to prompt to fix invalid flags
        project_key (str, optional): LaunchDarkly project key

    Returns:
        bool: True if all flags are valid or fixed, False otherwise
    """
    # Use provided project_key, client's project_key, or prompt for selection
    project_key = project_key or client.project_key
    if not project_key:
    ...
    print(f"\nValidating JSON feature flags in project '{project_key}'...")
    flags = client.get_feature_flags(project_key)
    ...
    for flag in flags:
        ...
        try:
            ...
            for variation in variations:
                value = variation.get("value")
                if isinstance(value, dict):
                    json_flags.append((flag, flag_details))
                    break
        except Exception as e:
            print(f"Error checking flag {flag.get('key')}: {str(e)}")
    ...
    print(f"\nFound {len(json_flags)} JSON feature flags to validate.")

    invalid_flags = []

    # Check each flag for validity
    for flag, flag_details in json_flags:
        flag_key = flag.get("key")
        flag_name = flag.get("name")
        variations = flag_details.get("variations", [])

        print(f"\nValidating flag: {flag_name} (key: {flag_key})")
        ...
        for i, variation in enumerate(variations):
            ...
            try:
                client.validate_tcp_port_json(value)
                print(f"  ✅ Variation '{var_name}' is valid")
            except ValueError as e:
                print(f"  ❌ Variation '{var_name}' is invalid: {str(e)}")
                invalid_variations.append((i, variation, str(e)))

        if invalid_variations:
            invalid_flags.append((flag_key, flag_name, variations, invalid_variations))
    ...
    print(f"\n❌ Found {len(invalid_flags)} invalid JSON feature flags.")

    if not fix_invalid:
        print("Run with --fix to fix invalid flags.")
        return False

    # Fix invalid flags
    print("\nFixing invalid flags...")

    for flag_key, flag_name, variations, invalid_variations in invalid_flags:
        print(f"\nFixing flag: {flag_name} (key: {flag_key})")
        print("Invalid variations:")
        for i, variation, error in invalid_variations:
            print(f"  {i+1}. {variation.get('name')}: {error}")

        # Let the user edit the variations
        print("\nOpening editor to fix flag variations...")
        edited_variations = edit_json_in_editor(variations)
        ...
        # Validate the edited variations
        all_valid = True
        for i, variation in enumerate(edited_variations):
            try:
                client.validate_tcp_port_json(variation.get("value", {}))
            except ValueError as e:
                print(f"❌ Variation {i+1} is still invalid: {str(e)}")
                all_valid = False

        if not all_valid:
            print("❌ Some variations are still invalid. Skipping update.")
            continue

        # Confirm the update
        confirm = input(f"\nUpdate flag '{flag_name}' with fixed variations? (y/n): ")
        if confirm.lower() != "y":
            print("Update cancelled.")
            continue
